...
const FAQS = [
  {
    q: "PicoSure 跟一般皮秒差在哪？",
    a: "PicoSure 皮秒雷射搭載 755nm 波長搭配升級版蜂巢透鏡，相較常見的 1064nm 皮秒，對黑色素選擇性更高、術後恢復更快。蜂巢透鏡還能觸發 LIOB 效應，在不產生傷口的情況下促進真皮層膠原蛋白新生，達到「養膚」效果。",
  },
  {
    q: "術後需要請假休息嗎？",
  ...
  {
    q: "皮秒雷射打多了會讓皮膚變薄嗎？",
    a: "PicoSure 以物理壓力波取代熱效應，不破壞角質層與基底膜。臨床觀察顯示，定期保養的案例皮膚屏障反而更穩固。這和傳統熱效應雷射的作用機制有本質上的不同。",
  },
  {
    q: "第一次體驗需要多少預算？",
...
<template>
  <div class="pb-8">
    ...
    <p class="text-[13px] text-txt-2 leading-[1.7] font-light mb-5">
    ...
    <div class="grid grid-cols-2 gap-2.5 mb-9">
      ...
      >
        <span class="text-[24px] leading-none">{{ opt.icon }}</span>
        <span class="text-[13px] leading-tight mt-1">{{ opt.label }}</span>
      </button>
    </div>

    <!-- ── FAQ heading ── -->
    <div class="mb-3">
      <div class="text-[10px] tracking-[2px] text-teal font-medium mb-2">FAQ</div>
      <h2 class="text-[20px] font-medium text-txt-1 leading-[1.55]">
        多數人也想知道的事
      </h2>
    </div>
    ...
    <div class="mb-8">
      ...
      >
        ...
        >
          <div
            class="shrink-0 w-6 h-6 rounded-md bg-teal-faint flex items-center justify-center text-[13px] font-medium text-teal"
          >
            {{ openFaq === idx ? '−' : 'Q' }}
          </div>
          <div
            class="text-[13.5px] font-medium text-txt-1 leading-[1.55] flex-1 pt-0.5"
          >
            {{ faq.q }}
          </div>
        </button>
        ...
        >
          <p class="text-[13px] text-txt-2 leading-[1.85] font-light pt-3">
            {{ faq.a }}
          </p>
        </div>
      </div>
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <span class="text-[11px] text-txt-3">3 / 4</span>
    </div>
